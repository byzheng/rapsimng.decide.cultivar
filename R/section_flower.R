.section_flower_spec <- function() {
	list(
		name = "flower",
		title = "Flowering",
		description = "Flowering time summary of the cultivar suitability evaluation.",
...
.build_section_flower_metrics <- function(state) {
	...
	metric_def <- tibble::tibble(
		...
		description = c(
			"The average flowering time across all years for each cultivar.",
			"The standard deviation of flowering time across all years for each cultivar.",
			"The coefficient of variation of flowering time across all years for each cultivar, calculated as the standard deviation divided by the mean.",
			"The 5th percentile of flowering time across all years for each cultivar, representing an early flowering scenario.",
			"The 10th percentile of flowering time across all years for each cultivar, representing a very early flowering scenario.",
			"The 25th percentile of flowering time across all years for each cultivar, representing a below-average flowering time scenario.",
			"The median flowering time across all years for each cultivar, representing a typical flowering time scenario.",
			"The 75th percentile of flowering time across all years for each cultivar, representing an above-average flowering time scenario.",
			"The 90th percentile of flowering time across all years for each cultivar, representing a late flowering scenario.",
			"The 95th percentile of flowering time across all years for each cultivar, representing a very late flowering scenario."
		),
	...
	list(
		...
		description = "The summary statistics of flowering time across all cultivars and years."
	)
}
...
.render_flower_summary_table_caption <- function() {
	": Summary statistics of flowering time across cultivars. {#tbl-flower-summary}"
}
...
.document_flower_summary <- function(section, meta = NULL) {
	...
	list(
		...
		body = c(
			"<!--",
			"Narrative:",
			"- Goal: summarise flowering time patterns across cultivars",
			"- Context: flowering timing performance across cultivars (use table below)",
			"- Focus: relative earliness, variability, and stable flowering patterns",
			"- Key metrics: mean flowering time, CV, quantile spread",
			...
			"",
			"Flowering time distribution across cultivars shown using quantile-based boxplots.",
			"",
			"```{r}",
			"#| label: fig-flower-summary-plot",
			"#| fig-cap: 'Flowering summary across cultivars'",
			plot_data_lines,
			"cultivar_column <- names(flower_summary_data)[[1]]",
			"flower_summary_plot_data <- flower_summary_data |>",
			"    dplyr::rename(cultivar = dplyr::all_of(cultivar_column)) |>",
			"    dplyr::arrange(flower_mean) |>",
			"    dplyr::mutate(cultivar = forcats::fct_reorder(cultivar, flower_mean))",
			"ggplot2::ggplot(",
			...
			"    ggplot2::coord_flip() +",
			"    ggplot2::labs(y = \"Flowering time (days)\", x = \"Cultivar\")",
			"```"
		)
	)
}