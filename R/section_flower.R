...
.document_flower_summary <- function(section, meta = NULL) {
	...
	list(
		...
		body = c(
            "<!--",
            "Narrative:",
            "- Goal: describe flowering time patterns across cultivars",
            "- Context: flowering timing across cultivars (use table below)",
			...
			"    ggplot2::coord_flip() +",
			"    ggplot2::labs(y = \"Flowering time (Days After Sowing)\", x = \"Cultivar\")",
			"```"
		)
	)
}