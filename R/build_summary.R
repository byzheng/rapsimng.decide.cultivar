...
.document_yield_summary <- function(section, meta = NULL) {
	...
	list(
		...
		body = c(
			...
			"<!--",
			"Narrative:",
			"- Goal: describe distribution patterns",
			"- Task: interpret boxplot features qualitatively",
			"- Context: yield distribution (boxplots with quantile distributions) across cultivars (use figure below, summary table above)",
			"- Focus: spread (IQR), median differences, and variability across cultivars",
			"- Limit: do not report or infer numeric yield values",
			"- Style: concise, farming decision oriented, decision-focused",
			"-->",
			"",
			"```{r}",
